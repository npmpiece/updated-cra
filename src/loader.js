...
const rootConfigFile = (() => {
  ...
  const loaded =
    configKeyIndex >= 0
      ? loadRawFromRoot(process.argv[configKeyIndex + 1]) || loadFromRoot(process.argv[configKeyIndex + 1]) || load(process.argv[configKeyIndex + 1])
      : loadFromPackageField('npmpiece-updated-cra-middleware') ||
...
const normalizeLoaded = x => {
  return type(x) === 'String' ? load(`@npmpiece/updated-cra-${x}`) || load(`piece-${x}`) || load(x) || load(`@piece/${x}`) || loadFromRoot(x) : x
}
...
const isolate = (scope, o) => reduce((accumulator, key) => (o[key] ? assoc(key, o[key], accumulator) : accumulator), {}, scope)
